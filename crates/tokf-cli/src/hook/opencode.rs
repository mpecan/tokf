...
// {{TOKF_BIN}} is replaced at install time with the resolved binary path.
...
fn write_plugin_file(plugin_dir: &Path, tokf_bin: &str) -> anyhow::Result<()> {
    ...
    // Escape for JS string literal (backslash and double-quote).
    let js_escaped = tokf_bin.replace('\\', "\\\\").replace('"', "\\\"");
