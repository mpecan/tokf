...
pub fn cmd_auth_login(rt: &Runtime) -> anyhow::Result<i32> {
    let base_url = client::server_url(rt);

    // Check if already logged in — may need ToS re-acceptance
    if let Some(auth) = credentials::load(rt) {
        return handle_existing_login(rt, &auth, &base_url);
    }
    ...
    let tos_version = prompt_tos_acceptance(rt, &base_url)?;
    ...
    poll_for_token(rt, &http_client, &base_url, &device_resp, tos_version)
}

/// If already logged in, check whether `ToS` re-acceptance is needed.
fn handle_existing_login(
    rt: &Runtime,
    auth: &credentials::LoadedAuth,
    base_url: &str,
) -> anyhow::Result<i32> {
    // Try to fetch current ToS version from server
    let Ok(client) = Client::unauthenticated(rt, base_url) else {
        eprintln!(
    ...
    let authed_client = Client::authed(rt)?;
    tos_client::accept_tos(&authed_client, tos_info.version)?;
    credentials::save_tos_accepted_version(rt, tos_info.version)?;
...
fn prompt_tos_acceptance(rt: &Runtime, base_url: &str) -> anyhow::Result<Option<i64>> {
    let Ok(client) = Client::unauthenticated(rt, base_url) else {
        ...
        match client::poll_token(http_client, base_url, &device_resp.device_code, tos_version) {
            Ok(client::PollResult::Success(token_resp)) => {
                ...
                if let Some(v) = tos_version {
                    credentials::save_tos_accepted_version(rt, v)?;
                }
                eprintln!();
                eprintln!("[tokf] Logged in as {}", token_resp.user.username);

                if let Some(auth) = credentials::load(rt) {
                    run_onboarding(rt, &auth);
                }
...
fn run_onboarding(rt: &Runtime, auth: &credentials::LoadedAuth) {
    let machine_available = prompt_machine_registration(rt, auth);

    if machine_available {
        prompt_usage_stats(rt);
    }
}

/// Ask the user whether to register this machine. Returns `true` if a machine
/// is registered (either newly or already was).
fn prompt_machine_registration(rt: &Runtime, auth: &credentials::LoadedAuth) -> bool {
    if machine::load(rt).is_some() {
    ...
    match crate::remote_cmd::register_machine(rt, auth) {
...
fn prompt_usage_stats(rt: &Runtime) {
...
pub fn cmd_auth_logout(rt: &Runtime) -> anyhow::Result<i32> {
    if credentials::remove(rt) {
...
#[allow(clippy::unnecessary_wraps)] // Returns Result for or_exit() consistency
pub fn cmd_auth_status(rt: &Runtime) -> anyhow::Result<i32> {
    match credentials::load(rt) {
        Some(auth) => {
...
pub fn cmd_auth_delete_account(rt: &Runtime) -> anyhow::Result<i32> {
