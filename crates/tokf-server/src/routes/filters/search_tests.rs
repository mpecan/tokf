...
async fn download_returns_toml_content(pool: PgPool) {
    ...
    // v1_hash is schema-independent (ADR-0002) — always present, always
    // formatted `v1:<64-hex>`.
    let v1_hash = json["v1_hash"].as_str().expect("v1_hash field present");
    assert!(
        v1_hash.starts_with("v1:") && v1_hash.len() == 67,
        "v1_hash must be `v1:<64-hex>`, got {v1_hash}"
    );
